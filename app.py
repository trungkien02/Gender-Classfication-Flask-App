...
# Load model
cnn_model = tf.keras.models.load_model(STATIC_FOLDER + "/models/" + "gender.h5")
...
def classify(model, image_path):
    ...
    pred = cnn_model.predict(preprocessed_imgage)
    if pred >= 0.5 :
        label = "Male"
        classified_pred = pred
    else:
        label = "Female"  
        classified_pred = 1 - pred 
    
    return label, classified_pred
